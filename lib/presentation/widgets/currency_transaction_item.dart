...
class CurrencyTransactionItem extends StatefulWidget {
  final Transaction transaction;
  final VoidCallback? onDelete;
  final Animation<double> animation;

  const CurrencyTransactionItem({
    super.key,
  ...
  @override
  State<CurrencyTransactionItem> createState() => _CurrencyTransactionItemState();
}

class _CurrencyTransactionItemState extends State<CurrencyTransactionItem>
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(
          'Asl valyuta: ${transaction.currencyCode}',
          style: Theme.of(context).textTheme.bodySmall?.copyWith(
...
class _TransactionAmount extends StatelessWidget {
          ...
          children: [
            Container(
              ...
              child: TweenAnimationBuilder<double>(
                tween: Tween<double>(begin: 0.0, end: transaction.amount),
                duration: AppConstants.mediumAnimation,
                curve: Curves.easeOutCubic,
                builder: (context, value, child) {
                  return Text(
                    _formatOriginalAmount(value),
                    key: ValueKey('original_${transaction.id}'),
            ...
            TweenAnimationBuilder<double>(
              ...
              builder: (context, value, child) {
                return Text(
                  viewModel.formatAmount(transaction.amount, currencyCode: transaction.currencyCode),
                  key: ValueKey('converted_${transaction.id}'),
                  style: Theme.of(context).textTheme.bodySmall?.copyWith(
  ...
  String _formatOriginalAmount(double amount) {
    switch (transaction.currencyCode) {
      case 'USD':
        return '${transaction.isIncome ? '+' : '-'}\$${amount.toStringAsFixed(2)}';
      case 'UZS':
        return '${transaction.isIncome ? '+' : '-'}${amount.toStringAsFixed(0)} so\'m';
      default:
        return '${transaction.isIncome ? '+' : '-'}${amount.toStringAsFixed(2)} ${transaction.currencyCode}';
    }
  }
}
